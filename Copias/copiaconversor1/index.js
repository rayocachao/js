...
app.post('/conversor', upload.single('xmlFile'), async (req, res) => {
  try {
    ...
    // Importar el módulo del conversor
    const { convertirXMLaJSON, guardarJSONenBaseDeDatos, descargarJSON } = require('./public/conversor');
    ...
    // Descargar el archivo JSON después de guardarlo en la base de datos
    const jsonFilePath = await descargarJSON(nuevoDocumento.toJSON());

    // Enviar una respuesta exitosa
    res.send('Conversión, guardado y descarga exitosos');
  } catch (error) {
    console.error(error);
    res.status(500).send('Error en la conversión, guardado o descarga');
  }
});
